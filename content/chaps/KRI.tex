...
$t \geq 0$ и $\bar h_{ij}(t) = 0$ при  $t < 0$ для $i = 1, \dots, m, j
= 1, \dots, m_i$. Из предыдущей системы, условий теоремы и неравенств
...
\end{array}
$$
$$(E|b_k|^{2p})^{1/(2p)} \leq \|b\|_{k^n_{2p}} \ \ (k=1,\dots,n), $$
$$
\mathrel {\mathop {v r a i \sup} \limits _{\varsigma < 0}}(E|\varphi
...
1, \dots ,l),$$ $$\chi _{ij} (\varsigma)\gamma (\varsigma) \leq \gamma
(\tau _{ij})\ \ (\varsigma \geq 0, \ i= 1,\dots,m, \ j= 1,\dots,m_i),$$
 $$ \chi _i (\tau)\gamma (\tau +1) \leq \gamma (d_i + 1) \ \ (\tau \in
N_+, \ i= 1,\dots,m),$$ $$\lambda < \max \{\lambda _i, i = 1, \dots,l,
-\ln (1-\lambda _ih), \ \ i = l+1,\dots,n \},$$
$$
\begin{array}{c}
...
_{t\geq 0}}((1 + \exp\{\ln(1-\lambda _kh)+ \lambda \} + \dots +$ $
\exp\{[t](\ln(1-\lambda _kh)+ \lambda )\} =  \\
 \sum \limits _{\tau=0
}^{\infty } \exp\{\tau(\ln(1-\lambda _kh)+ \lambda )\}=
\frac{1}{1-\exp\{\ln(1-\lambda_kh)+\lambda\}},  \ k = l+1,\dots,n,
\end{array}
$$
$$\gamma (t)\gamma (\bar h_{ij}(t))^{-1} \leq \gamma (\tau _{ij}) \, \ (t
\geq 0),  \ \ i = 1, \dots, m, \ j = 1, \dots, m_i,$$
$$\mathrel {\mathop
...
$i = 1,\dots,m,  k= l+1,\dots,n, r= 1,\dots,n,$ и наконец,
$$
\begin{array}{crl}
